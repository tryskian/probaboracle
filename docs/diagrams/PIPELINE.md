...
context and carries structural runtime values: prompt types, binary verdicts,
runtime settings, and local paths.

The agent path owns one minimal positive routing prompt and one stable positive
output contract. The final words are generated by the model in one node.
...
flowchart LR
  ...
  C["grammar-led correction<br/>diagnostic snapshot only"]
  D["config-history contamination risk"]

  E["Clean baseline candidate"]
  F["structural config<br/>positive routing prompt"]
